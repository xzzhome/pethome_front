<template>
  <section>
    ...
    <!--列表-->
    <!--
        :data="logininfos" 绑定数据
        v-loading="listLoading" 加载框，忙等框
         highlight-current-row 高亮提示
         @selection-change="selsChange" 选中事件
    -->
    <el-table :data="logininfos" highlight-current-row v-loading="listLoading"
      ...
      <el-table-column prop="type" label="类型" width="100" sortable>
        <template slot-scope="scope">
          <span style="color: orange" v-if="scope.row.type==0">管理员</span>
          <span style="color: blue" v-else-if="scope.row.type==1">用户</span>
        </template>
      </el-table-column>
      <el-table-column prop="disable" label="状态" width="100" sortable>
        <template slot-scope="scope">
          <span style="color: green" v-if="scope.row.disable==true">启用</span>
          <span style="color: red" v-else-if="scope.row.disable==false">禁用</span>
